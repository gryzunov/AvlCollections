...
namespace DataStructures
{
    ...
    /// </summary>
    public class CompactAvlTree2<T>
    {
        private static Node _dummyNode = null;
        private readonly IComparer<T> _comparer;
        private Node _root;

        public CompactAvlTree2()
        {
            _comparer = Comparer<T>.Default;
        }

        public CompactAvlTree2(IComparer<T> comparer)
        {
        ...
        private void RebalanceInsert2(ref Node pathTopRef, T value)
        {
            var path = pathTopRef;
        ...
        private void RebalanceInsert(ref Node pathTopRef, T value)
        {
            ...
            if (!path.IsBalanced)
            {
                ...
                if (compare > 0)
                {
                    if (path.Longer != Direction.Right)
                    {
                        path.Longer = Direction.None;
                        path = path.Right;
                    }
                    ...
                    {
                        var comp2 = _comparer.Compare(value, path.Right.Value);
                        if (comp2 > 0)
                        {
                            path = RotateRight(ref pathTopRef);
                        }
                        else
                        {
                            path = path.Right.Left;
                            var comp3 = _comparer.Compare(value, path.Value);
                            if (comp3 == 0)
                            {
                                path = RotateRightNone(ref pathTopRef);
                            }
                            else if (comp3 > 0)
                            {
                                path = RotateRightRight(ref pathTopRef);
                            }
                            else
                            {
                                path = RotateRightLeft(ref pathTopRef);
                            }
                        }
                    }
                }
                else if (compare < 0)
                {
                    ...
                    {
                        var comp2 = _comparer.Compare(value, path.Left.Value);
                        if (comp2 < 0)
                        {
                            path = RotateLeft(ref pathTopRef);
                        }
                        else
                        {
                            ...
                            if (comp3 == 0)
                            {
                                path = RotateLeftNone(ref pathTopRef);
                            }
                            else if (comp3 < 0)
                            {
                                path = RotateLeftLeft(ref pathTopRef);
                            }
                            else
                            {
                                path = RotateLeftRight(ref pathTopRef);
                            }
                        }
                    }
                }
        ...
        private static Node RotateLeft(ref Node pathTopRef)
        {
            var b = pathTopRef;
        ...
        private Node RotateRight(ref Node pathTopRef)
        {
            var b = pathTopRef;
        ...
        private static Node RotateLeftLeft(ref Node pathTopRef)
        {
            var b = pathTopRef;
            ...
            d.Longer = Direction.None;
            b.Longer = Direction.Right;
            f.Longer = Direction.None;

            return e;
        }

        private static Node RotateLeftRight(ref Node pathTopRef)
        {
            var b = pathTopRef;
            ...
            d.Longer = Direction.None;
            b.Longer = Direction.None;
            f.Longer = Direction.Left;

            return c;
        }

        private static Node RotateLeftNone(ref Node pathTopRef)
        {
            var b = pathTopRef;
        ...
        private static Node RotateRightRight(ref Node pathTopRef)
        {
            var b = pathTopRef;
            ...
            d.Longer = Direction.None;
            b.Longer = Direction.Left;
            f.Longer = Direction.None;

            return e;
        }

        private static Node RotateRightLeft(ref Node pathTopRef)
        {
            var b = pathTopRef;
            ...
            d.Longer = Direction.None;
            b.Longer = Direction.None;
            f.Longer = Direction.Right;

            return c;
        }

        private static Node RotateRightNone(ref Node pathTopRef)
        {
            var b = pathTopRef;
